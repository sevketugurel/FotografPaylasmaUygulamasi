...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // Eğer Kullanıcı henüz çıkış yapmamışsa tabBar sayfasını gösteriyoruz
        if guncelKullanici != nil {
            let board = UIStoryboard(name: "Main", bundle: nil)
            
            // Burada tabBarı alıyoruz ve convert edip UITabBar yapıyoruz
            let tabBar = board.instantiateViewController(identifier: "tabBar") as! UITabBarController
            
            //burada vc'nin kökünün ne olacağını tanımlıyoruz
